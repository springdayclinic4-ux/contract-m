...
export default function CompleteDailyContractTemplate({ data }: CompleteDailyContractTemplateProps) {
  ...
  const taxMethod = data.tax_method || 'business';
  ...
  // 세금 계산 - 일급 기준
  const dailyWage = parseFloat(data.wage_gross) || 0;
  const dailyNet = parseFloat(data.wage_net) || 0;
  ...
  return (
    <div className="bg-white p-12" style={{ fontFamily: "'Noto Serif KR', serif", fontSize: '14px', lineHeight: '1.8' }}>
      ...
      <div className="space-y-5">
        ...
        <section>
          ...
          <ol className="list-decimal pl-8 space-y-1">
            <li>
              근무장소: <span className="font-semibold">{data.hospital_name || ''}</span>
        ...
        <section>
          ...
          <ol className="list-decimal pl-8 space-y-1">
            ...
            <li>
              근로시간: <span className="font-semibold">{data.start_time || '09:00'}</span> 부터{' '}
              <span className="font-semibold">{data.end_time || '18:00'}</span> 까지
            </li>
            <li>휴게시간: <span className="font-semibold">{data.break_time || '13:00 ~ 14:00 (1시간)'}</span></li>
          </ol>
        </section>
        ...
        {hasSpecialConditions && (
          <section>
            ...
            <p className="pl-4 whitespace-pre-wrap">{data.special_conditions}</p>
          </section>
        )}
      ...
      {/* 제1페이지 서명란: 갑(병원) + 을(의사) */}
      <div className="mt-16">
        <p className="text-center mb-8 text-base font-medium">{today}</p>
        ...
        <div className="page-break pt-10 mt-16 border-t-2 border-gray-300">
          ...
          <div className="mt-16 text-center">
            ...
            <div className="flex justify-center items-center gap-4">
              <span className="font-bold text-lg">{data.hospital_name || ''} 대표 {data.director_name || ''}</span>
              <span className="text-gray-400 text-sm">(인/서명)</span>
            </div>
          </div>
        </div>
